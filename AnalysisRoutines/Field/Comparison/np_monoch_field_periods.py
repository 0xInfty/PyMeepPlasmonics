...
# Saving directories
folder = ["Field/NPMonoch/AuSphere/VacWatTest/TestPeriods/Vacuum", 
          "Field/NPMonoch/AuSphere/VacWatTest/TestPeriods/Water"]
home = vs.get_home()
...
period_results = [[vma.get_period_from_source(source_results[i][j], t_line[i][j], peaks_sep_sensitivity=.15)[-1] for j in range(len(series[i]))] for i in range(len(series))]
