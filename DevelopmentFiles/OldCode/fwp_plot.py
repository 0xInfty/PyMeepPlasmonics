...
def graph_3D(X, Y, Z, color_map='winter'):
    ...
    color_map='winter' : str or colormap, optional
        Color map specifier. Should be an allowed string. If not, 
        must be imported from matplotlib.cm
...
def graphs_3D(X, Y, Z, Z2, X2=None, Y2=None, 
    ...
    color_map=['winter', 'summer'] : list, optional
        Color map specifiers' list. Should be allowed strings. If not, 
        must be imported from matplotlib.cm
...
def animation_2D(X, Y, figure_id=None, new_figure=True, 
    ...
    ax.set_xlim((np.min(X), np.max(X, 'max')))
    ax.set_ylim((np.min(Y), np.max(Y, 'max')))
    line, = ax.plot([], [])
