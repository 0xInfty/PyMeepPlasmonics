...
# Saving directories
folder = ["Scattering/AuSphere/AllWatTest/10)TimeFactors/SecondTimeFactor"]
home = vs.get_home()

# Parameter for the test
test_param_string = "second_time_factor"
test_param_calculation = False
test_param_in_params = False
test_param_in_series = True
test_param_ij_expression = "second_time_factor[i][j]"
test_param_position = 0
test_param_name = trs.choose("Simulation additional time relative to normalization additional time", 
                             "Tiempo de simulación adicional relativo al tiempo adicional de normalización")
test_param_units = "" # Leave "" by default

# Sorting and labelling data series
sorting_function = [lambda l : vu.sort_by_number(l, test_param_position)]*2
series_label = trs.choose([lambda s : rf"Second Time Factor {vu.find_numbers(s)[test_param_position]:.0f}"]*2,
                          [lambda s : rf"Segundo Tiempo x{vu.find_numbers(s)[test_param_position]:.0f}"]*2)
...
plot_folder = "DataAnalysis/Scattering/AuSphere/AllWaterTest/SecondTimeFactor"
