...
for c, mr, col in zip(courant, mean_residual, colors):
    plt.plot(c, mr, '.', color=col, markersize=12)
plt.grid(True)
plt.legend(series_legend)
plt.xlabel("Courant factor")
...
for enl, cou in zip(elapsed_time, courant):
    ...
    second_courant.append( [] )
    second_flux_time.append( [] )
    second_build_time.append( [] )
    second_sim_time.append( [] )
    for e, c in zip(enl, cou):
        if len(e)==5:
            first_courant[-1].append( c )
            second_courant[-1].append( c )
            ...
            print(f"Unknown error in courant {c} of", cou)
