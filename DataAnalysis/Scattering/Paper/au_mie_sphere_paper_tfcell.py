...
# Saving directories
folder = ["AuMieSphere/AuMie/13)TestPaper/4)PaperJCFit/TestTFCell/AllVac450800"]
home = vs.get_home()
...
plot_make_big = False
plot_file = lambda n : os.path.join(home, "DataAnalysis/PaperTFCell" + n)
...
time_factor_cell = [p["time_factor_cell"] for p in params[0]]

plt.title("Difference in scattering maximum for " + plot_title)
plt.plot(time_factor_cell, dif_max_wlen, '.', markersize=12)
plt.grid(True)
...
for enl, tfc in zip(elapsed_time, time_factor_cell):
    ...
    second_time_factor_cell.append( [] )
    second_flux_time.append( [] )
    second_build_time.append( [] )
    second_sim_time.append( [] )
    for e, tf in zip(enl, tfc):
        if len(e)==5:
            first_time_factor_cell[-1].append( tf )
            second_time_factor_cell[-1].append( tf )
